...
<script type="text/javascript">
	$(function() {
		...
		$("[name=privilegeIds]").click(
				function() {
					//自己选中时，把所有的下级权限也同时选中或者取消
					$(this).siblings("ul").find("input").attr("checked",
							this.checked);
					//当选中一个权限时，也同时选中所有的直系上级权限
					if (this.checked) {
						$(this).parents("li").children("input").attr("checked",
								true)
					} else {
						//当取消一个权限时，同级没有选中的权限了，也就取消它的上级权限，再向上也是如此
						...
						<thead>
							<tr align="LEFT" valign="MIDDLE" id="TableTitle">
								<td width="300px" style="padding-left: 7px;">
									<!-- 全选 --> <input id="cbSelectAll" type="checkbox"
...
--%> <%-- 直接写HTML，并自行实现回显效果 --%>
									<ul id="root">
										<s:iterator value="#privilegeList">
											<li>
												<!-- 为什么要用EL表达式不用OGNL表达式？这里标签使用的是普通的input标签  而不是s:select标签 -->
												<!-- 回显 --> <input type="checkbox" name="privilegeIds"
												value="${id}" id="cb_${id}"
