...
public class AnchorTag extends AbstractClosingTag {
    ...
    /**
     * 标签读取结束时对标签的内容进行判断，通过则显示，不通过则隐藏
     *
     */
    @Override
    public int doEndTag() throws JspException {
	  ...
	  //用户点点击的当前URL，如果后面有参数就把参数去掉
	  String privUrl=action;
	  int fos=privUrl.indexOf("?");
	  if(fos>-1){
		  //截取字段
		  privUrl=privUrl.substring(0,fos);
	  }
	  //跟据当前的   权限URL 判断当前用户的权限
	  if(user.hasPrivilegeByPrivilegeUrl(privUrl)){
		  //验证通过，放行
		  return super.doEndTag();
	  }else{
		  //验证不通过，就不放行，跳过当前的标签，继续执行之后的标签
		  return EVAL_PAGE;
	  }
    }
